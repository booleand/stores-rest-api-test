...
class ItemTest(BaseTest):
    def setUp(self):
        ...
        with self.app() as client:
            with self.app_context():
                ...
                auth_token = json.loads(auth_request.data.decode('utf-8'))['access_token']
                self.access_token = {'Authorization': 'JWT {}'.format(auth_token)}
    ...
    def test_put_update_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                ItemModel('test', 4.99, 1).save_to_db()
                self.assertEqual(ItemModel.find_by_name('test').price, 4.99)
                resp = client.put('/item/test', data={'price': 1.99, 'store_id': 1})
